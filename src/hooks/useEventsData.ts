    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        if (location && location.lat !== undefined && location.lon !== undefined) {
          ...
          console.log('📍 [useEventsData] Using precise coordinates:', { lat: location.lat, lon: location.lon, source: location.source });
        } 
        // Priority 2: Use IP geolocation as fallback
          ...
          console.log('📍 [useEventsData] Fallback to IP geolocation:', location.ip);
        } 
        // Priority 3: Auto-detect IP if no location available
        else {
          filters.ip = 'auto';
          console.log('📍 [useEventsData] No location data, using IP auto-detection');
        }
        ...
        if (city || state) {
          ...
          delete filters.lon;
          delete filters.ip;
          console.log('📍 [useEventsData] Using text search override:', filters.q);
        }
