...
export default function CheckoutFormV9({ ticketData, eventData, onSuccess, onError }: CheckoutFormV9Props) {
  ...
  const calculateOrderDetails = useCallback(async () => {
    if (!ticketData.ticketGroupId) return
    ...
    try {
      ...
      const response = await api.post(
        `/checkout/calculate`,
        {
          ...
          retailUnitPrice: ticketData.pricePerTicket,
      ...
      if (response.data.success) {
        ...
        if (taxQuote) {
          setTaxAmount(taxQuote.tax_amount || 0)
  ...
  }, [ticketData, formData.billingAddress, formData.zipCode, onError])
    ...
    <div className="max-w-2xl mx-auto bg-gray-800 rounded-lg p-6">
      <div className="mb-6">
        ...
        <div className="bg-gray-700 rounded-lg p-4">
          ...
          <div className="flex justify-between items-center text-sm text-gray-300 mb-2">
            <span>Section {ticketData.section}, Row {ticketData.row}</span>
            <span>${ticketData.pricePerTicket.toFixed(2)} each</span>
          </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Billing Address (for physical tickets) */}
